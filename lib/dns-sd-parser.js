...
class DnsSdParser {
    ...
    * ---------------------------------------------------------------- */
    ...
    _parseLabel(buf, offset, visitedOffsets = new Set(), depth = 0) {
        ...
        while (true) {
            ...
            if ((label_len & 0b11000000) === 0b11000000) {
                // Bounds check before reading 2 bytes for pointer
                if (offset + length + 1 >= buf.length) {
                    invalid = true;
                    break;
                }
                let i = buf.readUInt16BE(offset + length) & 0b0011111111111111;
                // Prevent infinite recursion from pointer loops
                if (visitedOffsets.has(i)) {
                    invalid = true;
                    break;
                }
                // Prevent forward pointers (must point backwards)
                if (i >= offset + length) {
                    invalid = true;
                    break;
                }
                visitedOffsets.add(i);
                let parsed = this._parseLabel(buf, i, visitedOffsets, depth + 1);
                if (parsed) {
    ...
    _parseRdataPtr(buf, offset, len) {
        // Pass new visitedOffsets set to prevent pointer loop attacks
        const parsed = this._parseLabel(buf, offset, new Set([offset]), 0);
        if (parsed) {
            return parsed['name'];
        } else {
            return null;
    ...
    _parseRdataSrv(buf, offset, len) {
        ...
        // Pass new visitedOffsets set to prevent pointer loop attacks
        const parsed = this._parseLabel(buf, offset + 6, new Set([offset + 6]), 0);
        let target = null;
        if (parsed) {
            target = parsed['name'];
        }
